...
function build_jvm_image() {
  ...
  cd ../
  compdb -p jdk list >compile_commands_$image_variant.json
  mv compile_commands_$image_variant.json jdk
  cd - || exit
}
...
function run_make() {
  if [[ $1 == "all" ]]; then
    ...
    for variant in "${variants[@]}"; do
      ...
      cd ../
      compdb -p jdk list >compile_commands_$variant.json
      mv compile_commands_$variant.json jdk
      cd - || exit
    done
  else
    local variant=$1
